    ...
    with closing(app.connect()) as conn:
        ...
        if app.calendar is not None:
            try:
                ...
                log.exception("follow-ups deferred: calendar could not be checked")
                return 0
        ...
        for plan in due:
            ...
            with transaction(conn):
                # Recheck under the write lock: a manual job can race the scheduler.
                if plans.get(conn, plan.id).followed_up_at is not None:
                    continue
