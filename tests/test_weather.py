...
from familydb.integrations.open_meteo import DayForecast, OpenMeteo, parse_daily, summarize_code
...
def test_parse_daily_and_codes() -> None:
    ...
    assert short[0].code is None and short[0].high is None
...
def test_open_meteo_url_and_cache(settings, monkeypatch) -> None:
    ...
    assert "timezone=America%2FVancouver" in url
    calls: list[str] = []
