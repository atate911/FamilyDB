...
- Travel is from home unless the family says where they are. A place named in words (`near`: "downtown", "the Pearl") is looked up with the free geocoder, and a match far from home is retried with the home area. A location shared on Telegram (the paperclip, then Location, live or not) is kept for the member and used for three hours: by `near: here`, and without asking for questions about now or today. The coordinates stay with the engine; the model sees only "Sam's shared location". The web page cannot read a phone's position without a script, so there the family says where they are.
...
| Double submits | **Decided: a token per form, in memory** | The content policy forbids the script that would disable a button after one press. A token drawn with each form lets the server do it; the first post with a token does the work and later ones land where it went. In memory, because a duplicate arrives within seconds: a table would be a write on every form to guard against a restart in between. |
