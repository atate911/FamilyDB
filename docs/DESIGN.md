...
- A form whose repeat would do harm (a plan, an outcome, a message, a member) does its work once: each carries a token drawn with the page, and a double click, a refresh that resends or the back button lands where the first post went without doing anything again. A settings save needs none, since sending it twice sets the same values. Every form's post carries its origin because the page is served with `Referrer-Policy: same-origin`; under `no-referrer` browsers send `Origin: null`, which the page refuses, and nobody could sign in.
...
| Database | Open, recommend SQLite | One file on our own machine, trivial backups, FTS5 built in (section 3, "Own the data"). The page and the bot write to it at once, which write-ahead logging and short transactions handle; Postgres would be for many concurrent writers, which a family does not have. |
