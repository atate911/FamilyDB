...
- **Phase 4, surfaces.** In progress. The web page is built: the ideas list with search and filters, an idea in full with its place details, the restaurants, the plans, a shared-password gate and an optional HTTPS front for a public server, plus a status page and a settings page that configures the bot — provider, models, effort, home, schedules and the API keys — without editing a file or restarting anything. Still to come: editing ideas and plans from the page, optional extra channels, OpenClaw or Claude connectors as alternative front ends over the same tools.
...
src/familydb/
  ...
  web/                   __init__.py (the Flask factory), auth.py, routes.py, status.py,
                         settings.py (the only writer), fields.py, views.py, server.py, keys.py,
