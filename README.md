...
- A small web page shows the same list in a browser: search and filter the ideas, open one to see its hours, travel time and booking link, browse the restaurants, and see what is on the calendar. Ideas and plans are still only ever changed by messaging the bot; what the page can change is the bot itself — which model answers, how hard it thinks, where home is, when it speaks first, and the API keys — on a settings page, with a status page saying what is connected, what is waiting and what the last month cost.
