...
- A web page does the same things in a browser, for anyone who would rather not open Telegram. Ask it something on the chat page and the answer arrives there, from the same pipeline and with the same checks. Search and filter the ideas, open one to see its hours, travel time and booking link, browse the restaurants, and see what is on the calendar.
...
- What the page also changes is the bot itself — which model answers, how hard it thinks, where home is, when it speaks first, and the API keys — on a settings page, with a status page saying what is connected, what is waiting and what the last month cost.
